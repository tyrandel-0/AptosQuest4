...
export async function cellanaLock(privateKeys: string[], aptosClient: AptosClient, delay: DelayTimeRange) {
...
async function lockCELL(walletIndex: number, aptosAccount: AptosAccount, aptosClient: AptosClient, delay: number) {
...
async function getCELLBalance(aptosAccount: AptosAccount, aptosClient: AptosClient): Promise<string> {
    try {
        const provider = new Provider("mainnet" as Network)
        const coins = await provider.getAccountCoinsData(aptosAccount.address())
        for (let coin of coins.current_fungible_asset_balances) if (coin.asset_type == '0x2ebb2ccac5e027a87fa0e2e5f656a3a4238d6a48d93ec9b610d570fc0aa0df12') return String(coin.amount)
    } catch (error) {
